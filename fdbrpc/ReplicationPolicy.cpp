	...
	std::vector<LocalityEntry>	&				results )
{
	...
	if ((g_replicationdebug > 5) && (alsoServers.size())) {
	...
	for (auto& alsoServer : alsoServers) {
		...
		if ( g_replicationdebug > 5 ) {
			printf("Across !duplicate key: %-7s value: (%3d) %-10s entry: %s\n",
				   _attribKey.c_str(), value.present() ? value.get()._id : -1,
				   fromServers->valueText(value.get()).c_str(), fromServers->getEntryInfo(alsoServer).c_str());
		}
	}
