...
struct ParallelBackupAndRestoreCorrectnessWorkload : TestWorkload {
	...
	ParallelBackupAndRestoreCorrectnessWorkload(WorkloadContext const& wcx)
		: TestWorkload(wcx) {
	...
	virtual std::string description() {
		return "ParallelBackupAndRestoreCorrectness";
	}
	...
	ACTOR static Future<Void> doBackup(ParallelBackupAndRestoreCorrectnessWorkload* self, double startDelay, FileBackupAgent* backupAgent, Database cx,
	 ...
	 */
	ACTOR static Future<Void> attemptDirtyRestore(ParallelBackupAndRestoreCorrectnessWorkload* self, Database cx, FileBackupAgent* backupAgent, Standalone<StringRef> lastBackupContainer, UID randomID) {
	...
	ACTOR static Future<Void> _start(Database cx, ParallelBackupAndRestoreCorrectnessWorkload* self) {
		...
		if (self->agentRequest) {
			ParallelBackupAndRestoreCorrectnessWorkload::backupAgentRequests ++;
		}
		...
		try{
			...
			if ((g_simulator.backupAgents == ISimulator::BackupToFile) && (!ParallelBackupAndRestoreCorrectnessWorkload::backupAgentRequests)) {
...
int ParallelBackupAndRestoreCorrectnessWorkload::backupAgentRequests = 0;

WorkloadFactory<ParallelBackupAndRestoreCorrectnessWorkload> ParallelBackupAndRestoreCorrectnessWorkloadFactory("ParallelBackupAndRestoreCorrectness");
